...
/**
 * WS gateway cho chat real-time. Mirror notification.gateway.ts nhưng namespace `/messages`
 * và subscribe pattern `msg:user:*`.
 */
@WebSocketGateway({
  namespace: '/messages',
  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnModuleInit, OnModuleDestroy
{
  ...
  async onModuleInit() {
    ...
    await this.subscriber.psubscribe('msg:user:*');
    this.logger.log('Subscribed to Redis pattern msg:user:*');

    this.subscriber.on('pmessage', (_pattern, channel, raw) => {
      const userId = channel.split(':')[2];
      if (!userId) return;

      let payload: { type?: string; message?: any };
      try {
        payload = JSON.parse(raw);
      } catch {
        this.logger.warn(`Invalid message payload on ${channel}`);
        return;
      }
      ...
      this.server.to(room).emit('messages:unread-count-updated');
    });
  }
  ...
  async onModuleDestroy() {
    if (this.subscriber) {
      try {
        await this.subscriber.punsubscribe('msg:user:*');
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      if (!token) return this.rejectConnection(client, 'Thiếu access token');
